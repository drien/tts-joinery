        ...
        with click.progressbar(sentences, label="Chunking sentences") as sentence_list:
            for sentence in sentence_list:
                if (
                    ...
                    i += 1
                    chunked_input.append(
                        [sentence]
                    )  # Will fail for sentences > 4096 chars

    chunked_input = [" ".join(a) for a in chunked_input]

    print(
        f"Preparing to run TTS in {len(chunked_input)} chunks...({list(map(len, chunked_input))})"
    )
